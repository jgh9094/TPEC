...
def collect_results(base_dir):
    ...
    Args:
        base_dir: Path to the MutP directory
    ...
    # Traverse the directory structure: task_id/mutation_prob/replicate/
    ...
    for task_dir in sorted(base_path.iterdir()):
        ...
        # Iterate through mutation probabilities (P25, P50, P75, P100, etc.)
        for mutation_prob_dir in sorted(task_dir.iterdir()):
            if not mutation_prob_dir.is_dir():
                continue

            mutation_prob = mutation_prob_dir.name[1:]  # Extract number only: "P25" -> "25"

            # Iterate through replicates
            for replicate_dir in sorted(mutation_prob_dir.iterdir()):
...
def main():
    parser = argparse.ArgumentParser(
        description='Aggregate mutation probability experiment results into a CSV file.'
    )
    parser.add_argument(
        '--data_dir',
        type=str,
        default=None,
        help='Path to the MutP directory containing the results'
    )
    parser.add_argument(
        '--output',
        type=str,
