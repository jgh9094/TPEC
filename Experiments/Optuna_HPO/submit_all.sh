...
#
# Placement determines scope: at Experiments/Optuna_HPO it submits the whole factorial
# (2 Pop x 4 sampler x 6 model = 48 jobs); inside a Pop* folder it submits only that
# population's 24 jobs (submit one Pop at a time to avoid flooding the HPC).
...
# sampler, or model), letting you submit a subset without editing files.
set -euo pipefail
