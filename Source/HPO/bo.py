...
class BO:
    """
    BO Solver for hyperparameter optimization only.
    """
    def __init__(self,
                 model_config: Dict,
                 seed: int,
        ...
        self.total_evals = total_evals # total number of hard evaluations done (on true objective)
        self.top_candidates = top_candidates # number of top candidates to retain from sampled tpe set
        self.num_offspring = num_offspring # number of peseduo offspring to generate per cycle
    ...
    def run(self) -> None:
        ...
        current_set = self.evaluation(current_set)
        ...
        # keep track of best
        self.update_best_seen(current_set)
        ...
        while self.hard_eval_count < self.total_evals:
            # fit TPE model to tpe archive
            self.tpe.fit(self.tpe_archive, self.param_space, self.rng)

            # generate pseudo-offspring using TPE
            pseudo_offspring_p = self.tpe.sample(self.num_offspring * self.top_candidates, self.param_space, self.rng)
            assert len(pseudo_offspring_p) == self.num_offspring * self.top_candidates

            # reduce to top candidates based on expected improvement
            pseudo_offspring_p = [pseudo_offspring_p[i] for i in self.tpe.suggest_top_k(self.param_space,
                                                                                        pseudo_offspring_p,
                                                                                        self.top_candidates,
                                                                                        self.rng)]
            # convert parameters to scikit-learn compatible if needed
            pseudo_offspring_p = [self.param_space.eval_parameters(params) or params for params in pseudo_offspring_p]

            assert len(pseudo_offspring_p) == self.top_candidates

            # Evaluate current set of Individuals with Ray
            current_set = [Individual(params, self.param_space.get_model_type()) for params in pseudo_offspring_p]
            current_set = self.evaluation(current_set)
            self.hard_eval_count += len(current_set)
    ...
    def update_archive(self, evaluated_individuals: List[Individual], window: int) -> None:
        ...
        for ind in evaluated_individuals:
            ...
            arch_ind.set_val_performance(ind.get_val_performance())  # Set performance on new individual
